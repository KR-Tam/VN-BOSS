...
const WORKER_VERSION = '2026-07-06-news-numberrules';
    ...
    messages: [
      ...
      },
      {
        ...
        content: [
          ...
          '   • 복합 단위(가장 자주 틀림): "nghìn tỷ"=천 × 십억=1,000 tỷ(=1조). "triệu tỷ"=백만 × 십억=1,000,000 tỷ(=1,000조). 예: "1 triệu tỷ đồng"=100만 tỷ 동(약 1,000조 동), "400 nghìn tỷ đồng"=40만 tỷ 동(=400조 동).',
          '   • 큰 금액은 반드시 "tỷ(십억) 동" 단위로 표기하라(예: "약 800,000 tỷ 동", "약 100만 tỷ 동"). 한국식 "조/억"으로 임의 환산하지 말 것. 굳이 조를 쓰려면 tỷ 값을 괄호로 함께 적어 검증 가능하게 하라.',
          '   • tỷ를 "조"로, triệu를 "만"으로 잘못 줄이지 마라. 원문 숫자를 그대로 유지하고 변환을 한 번 더 검산하라.',
          '   • 기사에 나온 수치·공식·계수·기준금액·구체 예시 금액은 summaryKo에 빠짐없이 포함하라.',
          '- summaryKo: 기사 핵심을 4~6문장으로 정리(누가·무엇을·왜·핵심 수치·영향). 위 숫자 규칙을 지켜라. 기사에 여러 항목(대상 업종, 리스크 유형 등)이 나열되면 하나로 줄이지 말고 나온 항목을 모두 포함하라(예: 전자상거래·디지털 비즈니스·이전가격·장기 적자 신고·송장 리스크·자금 흐름·관계사 거래 등).',
          ...
          '   (1) 문제 상황 예시: 이 사안에서 어떤 사장님이 어떤 구체적 상황에서 어떤 손해·불이익·리스크를 겪는지 가상의 시나리오 1개를 든다. 반드시 "예를 들어 ~수 있습니다"처럼 가정임을 드러내라. 기사에 없는 특정 인물·업소를 기사 사실인 것처럼 쓰지 마라(기사가 일반적 리스크를 설명하면 예시도 일반적 가정으로).',
